...
func (spd *SafetyPlanDao) GetByID(safetyID int) *Model.SafetyPlan {
	...
	sp := Model.NewSafetyPlan(int(spuid), result[0][1], result[0][2], result[0][3], result[0][4], result[0][5], int(uc), int(clientuid), int(clinicianid))
...
func (spd *SafetyPlanDao) GetByUserID(userID int) []*Model.SafetyPlan {
	...
	for i := 0; i < len(result); i++ {
		...
		sp := Model.NewSafetyPlan(int(spuid), result[i][1], result[i][2], result[i][3], result[i][4], result[i][5], int(uc), int(clientuid), int(clinicianid))
		sp.SetSafetyID(int(spuid))
